...
class UriParam(val containerClass: ClassName, val paramTypeEnum: ParamTypeEnum) {
  ...
  val uriParamType = TypeSpec.classBuilder(uriParamTypeName)
    ...
    .addMethod(
      MethodSpec.methodBuilder("applyDefault")
        .addModifiers(PUBLIC)
        .returns(JavaTypes.Optional(JavaTypes.T))
        .addParameter(ParameterSpec.builder(JavaTypes.T, "passedValue", FINAL).build())
        .addStatement("return Optional.ofNullable(passedValue).or(() -> $L())", defaultValueField)
        .build()
    )
