...
sealed trait GqlSchemaDatatype {
  ...
  def valueFromJson(json: JsValue): String = {
    throw new UnsupportedOperationException(s"Unable to create default value for type $name json $json")
  }
}

// Namespace these to make it easier to refer to them without conflicting with Scala primitive types, etc
object GqlScheamDatatypes {

  var log = LoggerFactory.getLogger("GqlScheamDatatypes")

  sealed trait NativeDatatype extends GqlSchemaDatatype
  ...
  case object Boolean extends NativeDatatype {
    ...
    override def valueFromString(value: String): String = {
      ...
      valueFromJson(Json.parse(value))
    }

    override def valueFromJson(json: JsValue): String = {
      log.info(s"jsValue=$json")
  ...
  sealed trait UserDefinedDatatype extends GqlSchemaDatatype
  ...
  sealed abstract class ContainerDatatype(inner: GqlSchemaDatatype) extends GqlSchemaDatatype {
  ...
  case class List(inner: GqlSchemaDatatype) extends ContainerDatatype(inner) {
    override val apidocType = s"[${inner.apidocType}}]"
    override val shortName = s"[${inner.name}]"

    override def valueFromJson(json: JsValue) = {
      val defaults = json.as[JsArray].value.map {
        inner.valueFromJson
      }
      val initBlock =
        if (defaults.isEmpty) ""
  ...
  case class Map(inner: GqlSchemaDatatype) extends ContainerDatatype(inner) {
  ...
  case class Option(inner: GqlSchemaDatatype) extends GqlSchemaDatatype {
...
object GqlSchemaDatatype {

  var log = LoggerFactory.getLogger("GqlSchemaDatatype");

  def apply(datatype: Datatype): GqlSchemaDatatype = {

    datatype match {
      case Datatype.Primitive.Boolean => GqlScheamDatatypes.Boolean
      case Datatype.Primitive.Decimal => GqlScheamDatatypes.Decimal
      case Datatype.Primitive.Integer => GqlScheamDatatypes.Integer
      case Datatype.Primitive.Double => GqlScheamDatatypes.Double
      case Datatype.Primitive.Long => GqlScheamDatatypes.Long
      case Datatype.Primitive.Object => GqlScheamDatatypes.Object
      case Datatype.Primitive.JsonValue => GqlScheamDatatypes.JsonValue
      case Datatype.Primitive.String => GqlScheamDatatypes.String
      case Datatype.Primitive.DateIso8601 => GqlScheamDatatypes.DateIso8601
      case Datatype.Primitive.DateTimeIso8601 => GqlScheamDatatypes.DateTimeIso8601
      case Datatype.Primitive.Uuid => GqlScheamDatatypes.Uuid
      case Datatype.Primitive.Unit => GqlScheamDatatypes.Unit

      case Datatype.Container.List(inner) => GqlScheamDatatypes.List(GqlSchemaDatatype(inner))
      case Datatype.Container.Map(inner) => GqlScheamDatatypes.Map(GqlSchemaDatatype(inner))
      case Datatype.Container.Option(inner) => GqlScheamDatatypes.Option(GqlSchemaDatatype(inner))

      case Datatype.UserDefined.Model(name) =>
        val (ns, n) = parseQualifiedName(name)
        GqlScheamDatatypes.Model(ns, n)

      case Datatype.UserDefined.Enum(name) =>
        val (ns, n) = parseQualifiedName(name)
        GqlScheamDatatypes.Enum(ns, n)

      case Datatype.UserDefined.Union(name) =>
        val (ns, n) = parseQualifiedName(name)
        GqlScheamDatatypes.Union(ns, n)
    }
  }
  ...
  private def parseQualifiedName(name: String): (Option[String], String) = {
    name.split("\\.").toList match {
      case n :: Nil => (None, GraphQLUtil.toTypeName(n))
      case multiple =>
        val n = multiple.last
        val ns = multiple.dropRight(1).mkString(".")
        (Some(ns), GraphQLUtil.toTypeName(n))
    }
  }
}