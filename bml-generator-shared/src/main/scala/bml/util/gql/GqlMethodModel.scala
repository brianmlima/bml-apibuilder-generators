...
/**
  ...
  */
class GqlMethodModel(datatypeResolver: DatatypeResolver, resource: Resource, operation: Operation, methodPrefix: String) extends MethodModel {
  ...
  private def makeRequiredParams = {requiredParams.seq.map(makeParam)}

  private def makeOptionalParams = {optionalParams.seq.map(makeParam)}
