...
import bml.util.java.client.JavaRestTemplateClients
...
class JavaRestTemplateClientGenerator extends CodeGenerator {
  ...
  class Generator(service: Service, header: Option[String]) {
    ...
    def generateClient(): Seq[File] = {
      JavaRestTemplateClients.generateClient(service, nameSpaces)
    }
  }
