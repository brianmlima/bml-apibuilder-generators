...
class SpringServiceGenerator extends CodeGenerator with JavaPojoUtil {
  ...
  val springVersion: SpringVersion = SpringVersion.SIX
    ...
    def generateSourceFiles(): Either[Seq[String], Seq[File]] = {
      ...
      Right(
        generateServices(service) ++
          ...
          MethodArgumentNotValidExceptionHandler.get(springVersion, service, nameSpaces) ++
          StringToEnumConverters.enumConverters(springVersion, service, nameSpaces, service.enums) ++
